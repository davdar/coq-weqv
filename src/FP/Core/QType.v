...
Defined.
Notation "'λ'  x .. y → e" := (QMorphismA (fun x => .. (QMorphismA (fun y => e)) ..)).


Definition ap {τ₁ τ₂} : dom (τ₁ ⇒ τ₂) -> dom τ₁ -> dom τ₂ := rapply.
Infix "∙" := ap.
Infix "$" := ap (only parsing).
...
Ltac qproper_elim :=
  repeat (
    ...
    repeat (
      qproper1 ||
      match goal with
      | [ |- ?f ∙ ?x ≃ ?g ∙ ?y ] => apply (qproper_apply (f:=f) (g:=g) (x:=x) (y:=y))
      | [ |- ?f ∙ ?x ⊑ ?g ∙ ?y ] => apply (qmonotonic_apply (f:=f) (g:=g) (x:=x) (y:=y))
      end)).
...
Definition on {A B C} : dom ((B ⇒ B ⇒ C) ⇒ (A ⇒ B) ⇒ (A ⇒ A ⇒ C)) := λ o f x y → o ∙ (f ∙ x) ∙ (f ∙ y).